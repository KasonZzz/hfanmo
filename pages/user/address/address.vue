				...
				<view class="row" v-for="(row,index) in addressList" :key="index" @tap="select(row)">
					...
					<view class="center">
						<view class="name-tel">
							...
							<view class="tel">{{row.phoneNum}}</view>
							<view class="default" v-if="JSON.parse(row.status)">
								默认
							</view>
						</view>
						<view class="address">
							{{(JSON.parse(row.area)).label}} {{row.address}}
						</view>
					</view>
					<view class="right">
						<view class="icon bianji" @tap.stop="edit(row.id)">
	...
	export default {
		...
		methods: {
			...
			getAddress() {
				uni.request({
					method:"POST",
					url: getApp().globalData.websiteUrl + 'spaAppDomicile/queryDomicile',
					data: {
						"userId": this.userId
					},
					success: (res) => {
			...
			add() {
				uni.navigateTo({
					url: "edit/edit?type=add&userId=" + this.userId
				})
			},
			select(row) {
