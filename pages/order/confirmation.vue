			...
			<view class="row" v-for="(row,index) in buylist" :key="index">
				<view class="goods-info">
					...
					<view class="info">
						...
						<view class="spec">选择{{row.productDesc}} </view>
	...
	export default {
		...
		methods: {
			...
			getAddress(e) {
				uni.request({
					method: "POST",
					url: getApp().globalData.websiteUrl + 'spaAppDomicile/queryDomicileByIdAndStatus',
					data: {
						"userId": e,
						"status": "true"
					},
					success: (res) => {
						this.recinfo = res.data.data[0];
						this.recinfo.area = JSON.parse(this.recinfo.area)
					},
				});
			},
			...
			toPay() {
				...
				let len = this.buylist.length;
				for (let i = 0; i < len; i++) {
					this.buylist[i].domId = this.recinfo.id;
